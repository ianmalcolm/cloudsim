...
public class CloudSim {
	...
	private final Logger logger = LoggerFactory.getLogger(CloudSim.class);

	/** The Constant CLOUDSIM_VERSION_STRING. */
	private final String CLOUDSIM_VERSION_STRING = "3.0";

	/** The CIS object. */
	private CloudInformationService cis = null;
	 ...
	 * @post $none
	 */
	private void initCommonVariable(Calendar _calendar, boolean _traceFlag) throws Exception {
		initialize();
	 ...
	 * @post $none
	 */
	public void init(Calendar cal, boolean traceFlag) {
		try {
			...
			logger.error("CloudSim.init(): The simulation has been terminated due to an unexpected error");
	 ...
	 *            - the minimal period between events. Events within shorter
	 *            periods after the last event are discarded.
	 * @see gridsim.CloudSimShutdown
	 * @see CloudInformationService.CloudInformationService
	 * @pre numUser >= 0
	 * @post $none
	 */
	public void init(Calendar cal, boolean traceFlag, double periodBetweenEvents) {
	 ...
	 * @post $none
	 */
	public double startSimulation() throws NullPointerException {
	 ...
	 * @post $none
	 */
	public void stopSimulation() throws NullPointerException {
		try {
			runStop();
		} catch (IllegalArgumentException e) {
			throw new NullPointerException(
	...
	public boolean terminateSimulation() {
		running = false;
		logger.info("Simulation: Reached termination time.");
		return true;
	}
	 ...
	 */
	public boolean terminateSimulation(double time) {
		if (time <= clock) {
			return false;
		} else {
			terminateAt = time;
	 ...
	 * @post $none
	 */
	public List<SimEntity> getCloudResourceList() {
		if (cis == null) {
			return null;
		}
	...
	protected void initialize() {
		...
		waitPredicates = new HashMap<SimEntity, Predicate>();
		clock = 0;
		running = false;
	}
	 ...
	 *            The new entity
	 */
	public void addEntity(SimEntity e) {
		SimEvent evt;
		if (running) {
			// Post an event to make this entity
			evt = new SimEvent(SimEvent.TYPE.CREATE, clock, e, cis, SimEvent.TAG.NULL, e);
			future.addEvent(evt);
		}
		if (!entities.contains(e)) { // Only add once!
			entities.add(e);
	...
	public boolean runClockTick() {
		...
		if (future.size() > 0) {
			...
			queue_empty = true;
			running = false;
			logger.info("Simulation: No more future events");
		}
	 ...
	 *            the delay
	 */
	public void hold(SimEntity src, long delay) {
		SimEvent e = new SimEvent(SimEvent.TYPE.HOLD_DONE, clock + delay, src);
		future.addEvent(e);
		((SimEntity) src).setState(SimEntity.STATE.HOLDING);
	}
	 ...
	 *            the delay
	 */
	public void pause(SimEntity src, double delay) {
		SimEvent e = new SimEvent(SimEvent.TYPE.HOLD_DONE, clock + delay, src);
		future.addEvent(e);
		((SimEntity) src).setState(SimEntity.STATE.HOLDING);
	}
	 ...
	 *            the data
	 */
	public void send(SimEntity src, SimEntity dest, double delay, SimEvent.TAG tag, Object data) {
		if (delay < 0) {
			throw new IllegalArgumentException("Send delay can't be negative.");
		}

		SimEvent e = new SimEvent(SimEvent.TYPE.SEND, clock + delay, src, dest, tag, data);
		future.addEvent(e);
	}
	 ...
	 *            the p
	 */
	public void wait(SimEntity src, Predicate p) {
		((SimEntity) src).setState(SimEntity.STATE.WAITING);
		if (p != SimEvent.ANY) {
			// If a predicate has been used store it in order to check it
			waitPredicates.put((SimEntity) src, p);
		}
	}
	 ...
	 * @return the int
	 */
	public int waiting(SimEntity dest, Predicate p) {
		int count = 0;
		SimEvent event;
		Iterator<SimEvent> iterator = deferred.iterator();
		while (iterator.hasNext()) {
			event = iterator.next();
			if ((event.getDestination() == dest) && (p.match(event))) {
				count++;
			}
		}
		return count;
	 ...
	 */
	public SimEvent select(SimEntity src, Predicate p) {
		SimEvent ev = null;
		Iterator<SimEvent> iterator = deferred.iterator();
		while (iterator.hasNext()) {
			ev = iterator.next();
	 ...
	 * @return the sim event
	 */
	public SimEvent findFirstDeferred(SimEntity src, Predicate p) {
		SimEvent ev = null;
		Iterator<SimEvent> iterator = deferred.iterator();
		while (iterator.hasNext()) {
			ev = iterator.next();
	 ...
	 * @return the sim event
	 */
	public SimEvent cancel(SimEntity src, Predicate p) {
		SimEvent ev = null;
	 ...
	 */
	public boolean cancelAll(SimEntity src, Predicate p) {
		SimEvent ev = null;
	...
	private void processEvent(SimEvent e) {
		...
		case HOLD_DONE:
			...
			if (src == null) {
				...
				((SimEntity) src).setState(SimEntity.STATE.RUNNABLE);
			}
			break;
	 ...
	 * @return true, if successful otherwise.
	 */
	public boolean pauseSimulation(long time) {
		if (time <= clock) {
			return false;
		} else {
			pauseAt = time;
