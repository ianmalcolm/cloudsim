...
public class VmSchedulerTimeSharedOverSubscriptionTest {
	...
	public void testInit() {
		...
		assertEquals(PeList.getTotalMips(peList), vmScheduler.getAvailableRes(), 0);
		assertEquals(PeList.getTotalMips(peList), vmScheduler.getMaxAvailableRes(), 0);
		assertEquals(0, vmScheduler.getTotalAllocatedResForVm(vm1), 0);
	}
	...
	public void testAllocatePesForVm() {
		...
		assertTrue(vmScheduler.allocateResForVm(vm1, mipsShare1));

		assertEquals(PeList.getTotalMips(peList) - MIPS / 4, vmScheduler.getAvailableRes(), 0);
		assertEquals(PeList.getTotalMips(peList) - MIPS / 4, vmScheduler.getMaxAvailableRes(), 0);
		assertEquals(MIPS / 4, vmScheduler.getTotalAllocatedResForVm(vm1), 0);

		List<Double> mipsShare2 = new ArrayList<Double>();
		mipsShare2.add(MIPS / 2);
		mipsShare2.add(MIPS / 8);

		assertTrue(vmScheduler.allocateResForVm(vm2, mipsShare2));

		assertEquals(
				PeList.getTotalMips(peList) - MIPS / 4 - MIPS / 2 - MIPS / 8,
				vmScheduler.getAvailableRes(),
				0);
		assertEquals(
				PeList.getTotalMips(peList) - MIPS / 4 - MIPS / 2 - MIPS / 8,
				vmScheduler.getMaxAvailableRes(),
				0);
		assertEquals(MIPS / 2 + MIPS / 8, vmScheduler.getTotalAllocatedResForVm(vm2), 0);
		...
		assertEquals(PeList.getTotalMips(peList), vmScheduler.getAvailableRes(), 0);
		assertEquals(PeList.getTotalMips(peList), vmScheduler.getMaxAvailableRes(), 0);
		assertEquals(0, vmScheduler.getTotalAllocatedResForVm(vm2), 0);
	}
	...
	public void testAllocatePesForVmInMigration() {
		...
		assertEquals(PeList.getTotalMips(peList) - MIPS / 4, vmScheduler.getAvailableRes(), 0);
		assertEquals(PeList.getTotalMips(peList) - MIPS / 4, vmScheduler.getMaxAvailableRes(), 0);
		assertEquals(0.9 * MIPS / 4, vmScheduler.getTotalAllocatedResForVm(vm1), 0);
		...
		assertEquals(
				...
				vmScheduler.getMaxAvailableRes(),
				0);
		assertEquals(0.9 * MIPS / 2 + 0.9 * MIPS / 8, vmScheduler.getTotalAllocatedResForVm(vm2), 0);
	...
	public void testAllocatePesForVmShortageEqualsToAllocatedMips() {
		...
		assertTrue(vmScheduler.allocateResForVm(vm1, mipsShare1));
		assertEquals(3330, vmScheduler.getAvailableRes(), 0);
		assertEquals(170, vmScheduler.getTotalAllocatedResForVm(vm1), 0);

		assertTrue(vmScheduler.allocateResForVm(vm2, mipsShare2));
		assertEquals(1330, vmScheduler.getAvailableRes(), 0);
		assertEquals(2000, vmScheduler.getTotalAllocatedResForVm(vm2), 0);

		assertTrue(vmScheduler.allocateResForVm(vm3, mipsShare3));
		assertEquals(1320, vmScheduler.getAvailableRes(), 0);
		assertEquals(10, vmScheduler.getTotalAllocatedResForVm(vm3), 0);

		assertTrue(vmScheduler.allocateResForVm(vm4, mipsShare4));
		assertEquals(0, vmScheduler.getAvailableRes(), 0);
		assertEquals(1674, vmScheduler.getTotalAllocatedResForVm(vm4), 0);

		vmScheduler.deallocatePesForAllVms();

		assertEquals(3500, vmScheduler.getAvailableRes(), 0);
		assertEquals(3500, vmScheduler.getMaxAvailableRes(), 0);
	}
	...
	public void testAllocatePesForSameSizedVmsOversubscribed() {
		...
		assertTrue(vmScheduler.allocateResForVm(vm1, mipsShare1));
		assertEquals(0, vmScheduler.getAvailableRes(), 0);
		assertEquals(1000, vmScheduler.getTotalAllocatedResForVm(vm1), 0);

		assertTrue(vmScheduler.allocateResForVm(vm2, mipsShare2));
		assertEquals(0, vmScheduler.getAvailableRes(), 0);
		assertEquals(500, vmScheduler.getTotalAllocatedResForVm(vm1), 0);
		assertEquals(500, vmScheduler.getTotalAllocatedResForVm(vm2), 0);

		assertTrue(vmScheduler.allocateResForVm(vm3, mipsShare3));
		assertEquals(0, vmScheduler.getAvailableRes(), 0);
		assertEquals(333, vmScheduler.getTotalAllocatedResForVm(vm1), 0);
		assertEquals(333, vmScheduler.getTotalAllocatedResForVm(vm2), 0);
		assertEquals(333, vmScheduler.getTotalAllocatedResForVm(vm3), 0);

		vmScheduler.deallocatePesForAllVms();

		assertEquals(1000, vmScheduler.getAvailableRes(), 0);
		assertEquals(1000, vmScheduler.getMaxAvailableRes(), 0);
	}
